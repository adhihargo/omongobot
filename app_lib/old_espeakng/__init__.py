...
class ESpeakNG(object):
    ...
    def _espeak_exe(self, args, sync=False):
        ...
        logging.debug('old_espeakng: executing %s' % repr(cmd))
    ...
    def voices(self):
        ...
        logging.debug('old_espeakng: voices: %s' % res)
        ...
        for v in res:
            ...
            logging.debug('old_espeakng: voices: parts= %s %s -> %s' % (len(parts), repr(parts), repr(voice)))
            voices.append(voice)
